...
**Answer:** The date format changed and it now begins with the year. Also, R recognizes the change as reflected under the column name where "chr" has now become "date".
...
**Answer** I think that R has summed up all the expenditure made in the various months listed, and is presenting us with a total figure.
...
**Answer** The code is attempting to determine what percentage of the total votes were cast for Moore. This is being done by dividing Moore's votes by the total number of votes.
...
**Answer I really do not see any differences.**
...
**Answer: Here, percent Moore seems to be organized in ascending order. This makes it easier to know Moore's highest of lowest percentage votes.**
...
**Answer: similar functions to str_to_upper, which I found on Google, include:**

**str_to_lower: this makes characters in a column appear in lower case.**

**str_to_title: this makes characters appear in initial capitals.**
...
**Answer: The mutate statement is creating a new name or variabe for addresses that have the name of these selected states in them. For instance, all address that has Maryland in them automatically fall under the MD category through the use of case_when. It feels to me like the command is literally saying: "in any case when you see this state(whatever state is being included in the code), write it as this(whatever new variable name is given as a substitute).**
...
**Answer: The problem with the answer for me is that there are only two columns, and it is unclear what the second column {total) is calculating.**
...
1.  What is the total sum spent in Maryland?

2.  How many payments were made with the payee name SYED SAQIB ALI?

3.  In what state was the most money spent?
