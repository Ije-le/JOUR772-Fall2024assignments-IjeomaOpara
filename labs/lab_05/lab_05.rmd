...
```{r}
calls <- read_csv ("calls.csv", col_names = FALSE)|>
  clean_names() |>
...
A1. In 2022, there were two dates with the most overdose calls: June 14 and October 4. There were 23 calls each on those dates. There are 366 rows in the data set, and since this tallies with the number of days that exist in a year, I do not think there were any days with no over dose calls.
...
A2. My findings show that there are more overdose calls made during the weekends. Between Friday and Sunday overdose calls rise above 600, but during the weekdays, the calls stay between 500 and 600.
...
calls |>
  ...
  summarize(count =n()) |>
  arrange(desc(count)) |>
  mutate(pct_count=count/sum(count)*100)
```
...
A4. The idea that stands out to me from this lab is that more people die from drug overdose during the weekends than other days.
